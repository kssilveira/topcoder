...
class PassingGrade {
	...
	int pointsNeeded(vector <int> pointsEarned, vector <int> pointsPossible, int finalExam) {
		...
		//int goal = ceil((possible + finalExam) * 0.65);
		int goal = ((possible + finalExam) * 65);
		if(goal % 100) {
			goal = goal / 100 + 1;
		} else {
			goal /= 100;
		...
		//cout << "goal: " << goal << " earned: " << earned << endl;
		//cout << "final: " << finalExam << endl;
		if(goal - earned <= finalExam) {
			return max(goal - earned, 0);
		}
		return -1;
	}
