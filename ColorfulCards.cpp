...
bool notprime[1010] = {false};
vector <int> res;
int n, N;
string colors;

bool match2(int ic, int in, vector<int> cr)
{
	if(ic == n) {
		reverse(cr.begin(), cr.end());
		for(int i = 0; i < ic; i++) {
			if(res[i] != cr[i]) res[i] = -1;
		}
		return true;
	}
	for(int i = in; i >= 1; i--) {
		if((colors[ic] == 'R' && !notprime[i]) ||
			 (colors[ic] == 'B' && notprime[i])) {
			cr.push_back(i);
			if(match2(ic + 1, i - 1, cr)) return true;
			cr.pop_back();
		}
	}
	return false;
...
bool match(int ic, int in, vector<int> cr)
{
	if(ic == n) {
		for(int i = 0; i < ic; i++) {
			res[i] = cr[i];
	...
	for(int i = in; i <= N; i++) {
		if((colors[ic] == 'R' && !notprime[i]) ||
			 (colors[ic] == 'B' && notprime[i])) {
			cr.push_back(i);
			if(match(ic + 1, i + 1, cr)) return true;
			cr.pop_back();
		}
	}
	return false;
...
class ColorfulCards {
	public:
	vector <int> theCards(int tN, string tcolors) {
		colors = tcolors;
		N = tN;
		...
		n = (int) colors.length();
		res = vector<int> (n);
		match(0, 1, vector<int>());
		reverse(colors.begin(), colors.end());
		match2(0, N, vector<int>());
